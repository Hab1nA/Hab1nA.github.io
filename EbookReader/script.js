...
window.onerror = function (msg, url, line, column, err) {
    ...
    document.querySelector(".app .error .error-description").innerHTML = "请尝试刷新页面或使用其他浏览器（Chrome 或 Firefox），如果问题仍然存在，<a href=\"https://github.com/pgaskin/ePubViewer/issues\">请在此报告</a>。";
...
App.prototype.fatal = function (msg, err, usersFault) {
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "错误";
    document.querySelector(".app .error .error-description").innerHTML = usersFault ? "" : "请尝试刷新页面或使用其他浏览器（Chrome 或 Firefox），如果问题仍然存在，<a href=\"https://github.com/pgaskin/ePubViewer/issues\">请在此报告</a>。";
...
App.prototype.doReset = function () {
    ...
    this.qs(".book").innerHTML = '<div class="empty-wrapper"><div class="empty"><div class="app-name">电子书阅读器</div><div class="message"><a href="javascript:ePubViewer.doOpenBook();" class="big-button">打开本地文件</a></div></div></div>';
...
try {
    ...
    App.prototype.loadLibrary = function () {
        fetch('books.json')
            .then(resp => {
                if (!resp.ok) throw new Error('无法加载书籍列表');
                return resp.json();
            })
            .then(data => {
                ...
                if (data.books && data.books.length > 0) {
                    ...
                    data.books.forEach(book => {
                        ...
                        card.onclick = () => ePubViewer.openLibraryBook(book.filename);
                        card.innerHTML = `
                            <div class="library-card-cover">
                                ${book.cover ? '<img src="books/' + book.cover + '" alt="">' : '<span class="cover-placeholder">📖</span>'}
                            </div>
                            <div class="library-card-info">
                                <div class="library-card-title">${book.title}</div>
                                <div class="library-card-author">${book.author || '未知作者'}</div>
                            </div>
                        `;
            ...
            .catch(err => {
                ...
                document.getElementById('libraryGrid').classList.add('hidden');
                document.getElementById('libraryEmpty').classList.remove('hidden');
            });
    };

    App.prototype.openLibraryBook = function (filename) {
        var url = 'books/' + filename;
        this.doBook(url);
    };

    App.prototype.backToLibrary = function () {
        this.doReset();
        document.querySelector('.library-wrapper').classList.remove('hidden');
        document.querySelector('.reader-wrapper').classList.add('hidden');
    };

    App.prototype.showReader = function () {
        document.querySelector('.library-wrapper').classList.add('hidden');
        document.querySelector('.reader-wrapper').classList.remove('hidden');
    };
    ...
    if (ufn.startsWith("!")) {
        ufn = ufn.replace("!", "");
        document.querySelector('.reader-wrapper .open').style = "display: none !important";
    }
    if (ufn) {
        // URL 中有书籍参数，直接进入阅读器
        ePubViewer.showReader();
        fetch(ufn).then(resp => {
